...
namespace MeuCorre.Controllers
{
        ...
        [HttpPost]
            ...
            public async Task<IActionResult> CriarTag([FromBody] Application.UseCases.Tags.Commands.CriarTagCommand command)
            {
            ...
            [HttpPut]
            public async Task<IActionResult> TagCategoria([FromBody] AtualizarTagCommand command)
            {
                var (mensagem, sucesso) = await _mediator.Send(command);
                if (sucesso)
                {
